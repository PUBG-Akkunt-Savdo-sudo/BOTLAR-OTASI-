...
# Eski Python versiyalarida (3.10'dan past) "dict | None" kabi yozuvlar
# ishlab chiqilgan vaqtda (runtime'da) baholanganda TypeError beradi,
# chunki `type.__or__` faqat 3.10'da qo'shilgan. Quyidagi importdan keyin
# BARCHA annotatsiyalar oddiy matn sifatida saqlanadi va HECH QACHON
# runtime'da hisoblanmaydi — shu bilan bu xato butun fayl bo'ylab, bironta
# annotatsiyani qo'lda o'zgartirmasdan, butunlay yo'qoladi (Python 3.7+
# bilan mos, hozirgi eski versiyangizda ham ishlaydi).
...
class DBWriteQueue:
    ...
    def __init__(self, db_path: str):
        ...
        # DIQQAT: navbat BU YERDA yaratilmaydi! Bu obyekt fayl import
        # qilinganda (asyncio.run(main()) chaqirilishidan OLDIN, hali
        # HECH QANDAY event loop ishlamayotganda) yaratiladi. Python
        # 3.10'dan oldingi versiyalarda asyncio.Queue() qurilgan zahoti
        # "joriy" loop'ga bog'lanib qoladi — keyin asyncio.run(main())
        # yangi loop yaratganda, navbat ESKI (hech qachon ishlamagan)
        # loop'ga qolib ketadi va "attached to a different loop" xatosi
        # chiqadi. Shuning uchun navbat start() ichida — to'g'ri loop
        # ALLAQACHON ishlab turgan paytda — yaratiladi.
...
CLEANUP_RETENTION_DAYS = 90       # audit/fraud yozuvlari shu muddatdan eski bo'lsa o'chiriladi
CLEANUP_INTERVAL_SECONDS = 30 * 24 * 3600  # oyiga bir marta — og'ir/tez-tez kerak bo'lmagan ish

async def cleanup_loop(interval_seconds: int = CLEANUP_INTERVAL_SECONDS):
    """Oyiga bir marta ishlaydi, uchta narsani tozalaydi:
    1) O'chirilgan botlardan diskda qolib ketgan papka/log fayllar
       (managed_bots/bot_<id>/, bot_logs/bot_<id>.log) — bot allaqachon
       DB'dan o'chirilgan bo'lsa ham, disk fayllari avtomatik o'chmaydi edi.
    2) DB'da bot allaqachon o'chirilgan bo'lsa ham qolib ketgan "etim"
       qatorlar (bot_settings, bot_ai_monitor_state) — himoya chorasi
       sifatida, oddiy holatda db_delete_bot bularni allaqachon tozalaydi.
    3) CLEANUP_RETENTION_DAYS'dan eski admin_logs va fraud_events yozuvlari
       — bular faqat audit/monitoring uchun, moliyaviy ma'lumot (transactions,
       payment_transactions, users.balance) BUTUNLAY TEGILMAYDI."""
    await asyncio.sleep(30)
    while True:
        try:
            existing_ids = await db_get_all_bot_ids()
            _cleanup_orphaned_disk_files(existing_ids)
            await _cleanup_orphaned_db_rows(existing_ids)
            await _cleanup_old_audit_records(CLEANUP_RETENTION_DAYS)
            logger.info("cleanup_loop: tozalash tsikli tugadi")
        except Exception:
            logger.exception("cleanup_loop global xato")
        await asyncio.sleep(interval_seconds)


async def db_get_all_bot_ids() -> set:
    async with db_connect() as db:
        rows = await db.execute("SELECT id FROM bots")
        rows = await rows.fetchall()
        return {r[0] for r in rows}


def _cleanup_orphaned_disk_files(existing_ids: set) -> None:
    """DB'da endi mavjud bo'lmagan bot_id'larga tegishli managed_bots/
    papkalari va bot_logs/ log fayllarini diskdan o'chiradi."""
    managed_root = Path("managed_bots")
    if managed_root.exists():
        for entry in managed_root.iterdir():
            if not entry.is_dir() or not entry.name.startswith("bot_"):
                continue
            try:
                bot_id = int(entry.name[len("bot_"):])
            except ValueError:
                continue
            if bot_id not in existing_ids:
                try:
                    shutil.rmtree(entry)
                    logger.info(f"cleanup_loop: o'lik bot papkasi o'chirildi -> {entry}")
                except Exception:
                    logger.exception(f"cleanup_loop: {entry} papkasini o'chirishda xato")

    if LOGS_DIR.exists():
        for entry in LOGS_DIR.iterdir():
            if not entry.is_file() or not entry.name.startswith("bot_") or not entry.name.endswith(".log"):
                continue
            try:
                bot_id = int(entry.name[len("bot_"):-len(".log")])
            except ValueError:
                continue
            if bot_id not in existing_ids:
                try:
                    entry.unlink()
                    logger.info(f"cleanup_loop: o'lik bot logi o'chirildi -> {entry}")
                except Exception:
                    logger.exception(f"cleanup_loop: {entry} faylini o'chirishda xato")


async def _cleanup_orphaned_db_rows(existing_ids: set) -> None:
    """Himoya chorasi: agar biror bot qandaydir yo'l bilan (masalan qo'lda
    SQL orqali) o'chirilib, bog'liq qatorlar qolib ketgan bo'lsa, ularni
    tozalaydi. Oddiy holatda db_delete_bot bularni allaqachon o'chiradi."""
    async with db_connect() as db:
        rows = await db.execute("SELECT DISTINCT bot_id FROM bot_settings")
        settings_bot_ids = {r[0] for r in await rows.fetchall()}
        rows = await db.execute("SELECT DISTINCT bot_id FROM bot_ai_monitor_state")
        monitor_bot_ids = {r[0] for r in await rows.fetchall()}

    orphaned_settings = settings_bot_ids - existing_ids
    orphaned_monitor = monitor_bot_ids - existing_ids
    if not orphaned_settings and not orphaned_monitor:
        return

    statements = []
    for bot_id in orphaned_settings:
        statements.append(("DELETE FROM bot_settings WHERE bot_id = ?", (bot_id,)))
    for bot_id in orphaned_monitor:
        statements.append(("DELETE FROM bot_ai_monitor_state WHERE bot_id = ?", (bot_id,)))
    if statements:
        await write_queue.execute_transaction(statements)
        logger.info(f"cleanup_loop: {len(statements)} ta etim DB qatori tozalandi")


async def _cleanup_old_audit_records(retention_days: int) -> None:
    """FAQAT audit/monitoring yozuvlari (admin_logs, fraud_events) —
    CLEANUP_RETENTION_DAYS'dan eski bo'lsa o'chiriladi. Moliyaviy jadvallar
    (transactions, payment_transactions, users) BU FUNKSIYAGA KIRMAYDI."""
    cutoff = (utcnow() - timedelta(days=retention_days)).isoformat()
    _, admin_logs_deleted = await write_queue.execute(
        "DELETE FROM admin_logs WHERE created_at < ?", (cutoff,))
    _, fraud_events_deleted = await write_queue.execute(
        "DELETE FROM fraud_events WHERE created_at < ?", (cutoff,))
    if admin_logs_deleted or fraud_events_deleted:
        logger.info(
            f"cleanup_loop: admin_logs -{admin_logs_deleted}, fraud_events -{fraud_events_deleted} "
            f"({retention_days} kundan eski)"
        )
